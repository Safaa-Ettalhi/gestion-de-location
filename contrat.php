...
<body class="bg-gray-100 ">
                        ...
                        while ($row = $resultCont->fetch_assoc()) {
                          echo '
                                <tr class="hover:bg-gray-50 transition">
                                    ...
                                    <td class="border border-gray-200 px-4 py-2 text-center">' . $row['marque'] . ' ' . $row['modele'] . '</td>
    ...
    <div id="addContractModal" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white rounded-lg shadow-lg w-full max-w-2xl p-6">
            ...
            <form method="POST" action="./addContrat.php">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <?php
                            ...
                            while ($client = $clients->fetch_assoc()) {
                                echo "<option value='{$client['id']}'>{$client['nom_complet']}</option>";
                            }
                        ?>
                    </select>
                    </div>
                    <div>
                        ...
                        <select id="voiture" name="id_voiture" class="w-full border rounded-md p-2 focus:outline-none focus:ring-2 focus:ring-blue-500" required>
                            ...
                            <?php
                                $voitures = $conn->query("SELECT id, CONCAT(marque, ' ', modele) AS voiture FROM voitures WHERE statut = 'Disponible'");
                                while ($voiture = $voitures->fetch_assoc()) {
                                    echo "<option value='{$voiture['id']}'>{$voiture['voiture']}</option>";
                                }
                            ?>
                        </select>
                    </div>
                    ...
                    <div>
                    <label for="statut" class="block text-sm font-medium text-gray-700">Voiture</label>
